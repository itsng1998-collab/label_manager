...
## 현재 상태
- 진행 중: AppBar root `MenuAnchor`의 바깥 클릭 소비가 다른 아이콘의 첫 클릭을 삼켜 새 메뉴가 열리지 않는 원인을 확인했다. 바깥 클릭은 메뉴를 닫되 이벤트를 대상 아이콘에 전달하고, 공용 controller가 같은 클릭에서 이전 메뉴를 닫고 새 메뉴를 연다. 마지막 close 상태 보고는 다음 frame으로 유예해 같은 클릭의 새 open이 있으면 취소하므로 shortcut blocker가 흔들리지 않는다. 두 번째 아이콘 클릭 후 단일 `pump()`에서 메뉴 교체를 검증한다. 포맷 후 AppBar/controller/shortcut/lifecycle 회귀 `27 passed`, 수정 파일 diagnostics 0건, `C:\Flutter\bin\flutter.bat build windows --debug` 성공, `git diff --check` 통과. 관련 3개 파일만 stage/commit한다.
