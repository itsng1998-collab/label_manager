...
- 확정 기준: 복수 `TYPE_PRINTCOUNT` TColumn은 현재 label-size의 `(columnOrder, columnId)` 안정 정렬 결과 중 첫 번째만 발행매수 baseline으로 사용하고 나머지는 무시하며 command도 같은 정렬 결과를 snapshot한다.
- 확정 기준: commit 성공 후 자동증가 대상만 갱신하는 snapshot은 현재 label-size의 마지막 저장·조회 `TColumnContent` baseline view이며, DAO commit 값으로 대상 `(columnId,itemId)` entry만 교체한다. row edit, printer setting snapshot, 품목관리 draft working value와 무관한 baseline 값은 유지한다.
- 수정 예정: `doc/label_print_modify.txt`, `SESSION_HANDOFF.md`. 본문·pipeline·테스트·완료 조건을 함께 정리하며 DB migration, retry/recovery, lock, spooler/native 변경은 추가하지 않는다.
- `doc/label_print_modify.txt` 편집 완료: copies 0의 대화형 copy index 0 미리보기와 발행 unit/payload 제외를 분리하고, 복수 `TYPE_PRINTCOUNT`의 stable first 선택, commit 후 `TColumnContent` baseline view 대상 entry 교체를 본문·pipeline·테스트·완료 조건에 연결했다.
- `SESSION_HANDOFF.md` 편집 완료: 이번 병합의 확정 기준과 파일별 진행 상태를 기록했다.
- 검증 예정: 두 문서 diagnostics, 이전 포괄 `copies 0 materialize 제외`/모호한 `session snapshot` 표현과 새 계약 검색, `git diff --check -- doc/label_print_modify.txt SESSION_HANDOFF.md`. 문서만 변경하므로 Flutter test/analyze는 실행하지 않는다.
- 독립 재검토: 세 계약의 본문·pipeline·테스트·완료 조건 사이 실제 모순, 구현 불가능성, 빠진 필수 검증 0건이다.
- 검증 실행 직전: 폐기 표현과 새 계약 연결 검색, 두 문서 diagnostics를 확인했으며 `git diff --check -- doc/label_print_modify.txt SESSION_HANDOFF.md`와 대상 diff를 최종 확인한다.
...
- stage/commit 대상: `doc/label_print_modify.txt`, `SESSION_HANDOFF.md`. 기존 사용자 `lib/core/app.dart`, `doc/label_print.txt` 변경은 제외한다.
- 작업지시서 session·순서·validation 계약 커밋: `2ce4b59 라벨출력 세션과 발행 검증 계약 명확화`.
...
- 수정 예정: `doc/label_print_modify.txt`, `SESSION_HANDOFF.md`. 실제 inline editor open/commit 진행만 active editing으로 정의하고 focus/selection은 제외하며, label-print 초기화 전 target 전용 gate 실행, 차단 시 activation/load/preview/printer/command 부작용 없음과 안내 1회 테스트를 본문·구현 순서·완료 조건에 연결한다. package 수정, 자동 commit/save/cancel, DB recovery/migration은 추가하지 않는다.
- `doc/label_print_modify.txt` gate 계약 편집 완료: `_blockLabelPrintTabSelection()` 동등 전용 helper가 `label_print` target에서만 active editing/command busy/dirty를 가장 먼저 판정하고, 기존 공용 context-change gate의 다른 호출부는 유지하도록 명시했다.
- `doc/label_print_modify.txt` active-edit 계약 편집 완료: `FortuneTableEditingController.hasActiveEditing`을 inline editor open 또는 commit 진행으로 한정하고 focus/row selection은 제외하며 `ItemManageController`가 read-only로 위임하도록 했다.
- `doc/label_print_modify.txt` 차단 결과·테스트 편집 완료: selection 즉시 복원, 안내 1회, label-print activation/row load/preview/printer 조회/command 생성 미실행을 마우스/F3/programmatic 진입과 완료 조건에 연결했다.
...
- stage/commit 대상: `doc/label_print_modify.txt`, `SESSION_HANDOFF.md`. 기존 사용자 `doc/label_print.txt`, `lib/core/app.dart` 변경은 제외한다.
- 작업지시서 병합 커밋: `36965a7 라벨출력 지시서 4차 권장안 확정`.
...
- stage/commit 대상: `doc/label_print_modify.txt`, `SESSION_HANDOFF.md`. 기존 사용자 `doc/label_print.txt`, `lib/core/app.dart` 변경은 제외한다.
- 작업지시서 병합 커밋: `c5c49ce 라벨출력 지시서 2차 권장안 확정`.
...
- 우선 검증: `C:\Flutter\bin\flutter.bat test test\label_column_save_test.dart`.
- `lib/models/label_column_save.dart`: label main/check/GS1 info/contain key range 잠금, 보조 원본 projection·비교, 레거시 NULL 정규화와 optional main check 충돌 검사를 추가했다.
- `lib/models/label_column_candidates.dart`: customer key range 잠금과 keyword/name NULL 정규화를 추가했다.
- `test/label_column_save_test.dart`: main/보조/customer 잠금, 보조 projection, NULL 정규화와 optional main check variant 계약을 추가했다. DAO 테스트 13개 통과.
- `doc/user_item_modify.txt`: snapshot 검사부터 DML까지 잠금 유지와 보조 값 비교 정책을 반영했다. 다음 검증: 관련 모델/transaction 테스트와 전체 analyze/test.
- 관련 `label_column_save/edit/candidates`, `db_transaction` 테스트 37개 전체 통과. 변경 Dart 파일 IDE 진단 없음, `git diff --check` 통과.
- 전체 검증 예정: `C:\Flutter\bin\flutter.bat analyze`, `C:\Flutter\bin\flutter.bat test`.
- 전체 `C:\Flutter\bin\flutter.bat analyze`: 이슈 없음. 다음 검증: `C:\Flutter\bin\flutter.bat test`.
...
- `lib/models/label_column_edit.dart`, `lib/models/label_column_candidates.dart`: 통합 save command와 적용 session 재편집 snapshot API를 추가했다.
- `lib/models/label_column_save.dart`, `lib/home_page_manager.dart`: 두 DAO statement를 하나의 `DbClient.transaction()`으로 저장하는 callback 경계를 연결했다.
- `test/label_column_edit_dialog_test.dart`, `test/label_column_save_test.dart`: 사용자 적용 직후 DB 미호출, 적용 후보 즉시 추가, 하단 최종 저장, 변경 취소 질의, 단일 transaction statement 목록 계약을 검증한다.
- focused 검증 `C:\Flutter\bin\flutter.bat test test\label_column_edit_dialog_test.dart test\label_column_save_test.dart`: 23개 전체 통과. 다음 검증: `C:\Flutter\bin\flutter.bat analyze`, `C:\Flutter\bin\flutter.bat test`.
- 전체 `C:\Flutter\bin\flutter.bat analyze`: 이슈 없음. 다음 검증: `C:\Flutter\bin\flutter.bat test`.
- 루트 `C:\Flutter\bin\flutter.bat test`: 369개 전체 통과. 다음 단계: `third_party/fortune_sheet/build/` 정리, IDE 진단과 diff 점검, 관련 파일만 stage/commit.
- 통합 command assertion 보강 후 focused 23개 재통과. `third_party/fortune_sheet/build/` 정리 완료, 관련 Dart 파일 IDE 진단 없음, `git diff --check` 통과.
- stage 대상: `doc/user_item_modify.txt`, 관련 lib 5개, test 2개. 사용자 `label-column-add` 들여쓰기와 `lib/core/app.dart`는 제외하며 기능 커밋 후 들여쓰기를 working tree에 복원한다.
- 기능 커밋: `e9570fa 라벨 항목 편집 저장 경계 통합`. 사용자 들여쓰기 변경은 working tree에 복원했고 `lib/core/app.dart`와 함께 커밋에서 제외했다.
...
- compact dropdown suffix 영역을 32×28로 고정하고 화살표를 가로·세로 정중앙에 배치하며 회귀 테스트를 보강한다.
- compact `DropdownMenu`의 중복 padding과 과대 trailing child가 원인이어서 `isCollapsed`와 20px icon으로 정리하고, 홀수 행 경계의 세로 0.5px를 보정했다.
- 다이얼로그 focused 테스트 13개 통과. 다음 검증: `C:\Flutter\bin\flutter.bat analyze`, `C:\Flutter\bin\flutter.bat test`.
- `C:\Flutter\bin\flutter.bat analyze` 이슈 없음. 다음 검증: `C:\Flutter\bin\flutter.bat test`.
- 루트 `C:\Flutter\bin\flutter.bat test` 367개 전체 통과. 다음 단계: 테스트 캐시 정리, IDE 진단과 diff 점검, 관련 hunk만 stage/commit.
- `third_party/fortune_sheet/build/` 정리 완료. 관련 파일 IDE 진단 없음, `git diff --check` 통과. 사용자 `label-column-add` 들여쓰기 변경과 `lib/core/app.dart`는 stage/commit에서 제외한다.
- 수정 파일: `lib/page_home/label_column_edit_dialog.dart`, `test/label_column_edit_dialog_test.dart` (완료). 기능 커밋: `4287da9 라벨 속성과 드롭다운 표시 밀도 보정`.
...
- 임시 산출물 `third_party/fortune_sheet/build/` 정리 완료. 최종 Dart 진단 이슈 없음, `git diff --check` 통과.
- stage/commit 대상: `lib/page_home/label_column_edit_dialog.dart`, `test/label_column_edit_dialog_test.dart`, `SESSION_HANDOFF.md`. unrelated `lib/core/app.dart`는 제외한다.
- 기능 커밋: `8fba70b 라벨 항목 편집 드롭다운과 테이블 폭 보정`.
...
- 기능 커밋 완료: `51a6e57` (`공용라벨 초기 로드 진행 표시 종료 보정`). 원격 push와 배포 작업은 수행하지 않았다.
...
- 로컬 커밋 완료: `e76e78d` (`작업지시서 항목 편집 DB 계약 보정`). `doc/user_item_modify.txt`, `SESSION_HANDOFF.md`만 포함했고 unrelated `.vscode/settings.json`, `lib/core/app.dart`는 제외했다.
...
- 커밋 완료: `8a5f0f1` (`작업지시서 자동 검토 범위 제한 명확화`). 대상은 `doc/user_item_modify.txt`, `SESSION_HANDOFF.md`이며 기존 unrelated `.vscode/settings.json`, `lib/core/app.dart`는 제외했다.
- 사용자 확정에 따른 작업지시서 재작성 완료: `doc/user_item_modify.txt`를 449줄의 UI/CRUD 중심 문서로 통합했다. blocking modeless UI, working copy와 stable draft key, 타입별 속성·검증, 고정/사용자 후보, reorder/userItemEdit mode, 단일 parameterized transaction, `OUTPUT INSERTED` ID mapping, check/min/content/GS1/status 처리, 오류 전달·가능한 rollback·working copy 유지를 남겼다. trigger fingerprint/metadata 인증, DDL·table-wide lock, capability/cache 상태기계, 품목 command·journal·market/delete 재설계, post-commit 전용 예외·force reload와 관련 테스트·완료 조건은 제거했다. 사용자 추가 확인 사항 없음.
...
- 사용자 확인 필요 여부: 없음. 기존 확정 원칙에 따라 이번 command가 새 중복·예약 keyword 위반을 만들 때만 차단하고 기존 위반 데이터 정리를 강제하지 않는다. 활성 INSERT trigger 없음은 자동 미지원이 아니라 command별 명시 보조행 생성 가능 여부로 판정하며, 레거시 source/runtime/schema 무변경과 레거시·새 앱 저장 비동시 운용 전제는 유지한다.
- 수정 예정 `doc/user_item_modify.txt`: trigger capability를 `없음/검증된 v1·v2/미확인·추가`로 구분하고 identifier/header 의미를 보존하는 fingerprint로 고친다. keyword 검증을 baseline 대비 신규 위반 차단으로 제한하고, 12.6의 현재 상태 오기를 선행 구현 요구로 정정한다. 직접 의존 변경과 별도 선행 정리 가능한 변경의 구현 단계를 분리해 본문·테스트·완료 조건을 함께 갱신한다. 미검증.
- 편집 완료 `doc/user_item_modify.txt`: trigger capability를 parent별 `none/validatedLegacy/unsupported`로 분리하고 `none`은 컬럼 content/order 및 품목 필수 보조행을 DAO가 명시 완성할 수 있는지 command별로 판정하도록 했다. fingerprint는 CREATE 동사만 canonicalize하고 identifier 원문 case와 header option을 보존한다. 신규/keyword 변경 command가 만드는 새 중복·예약어 위반만 차단하고 기존 untouched 위반은 무관한 수정·삭제·reorder를 막거나 자동 정리하지 않도록 본문·DAO/통합 테스트·완료 조건을 연결했다. 각 편집 직후 diagnostics 오류 0건이다.
- 현재 상태·구현 단계 정정 `doc/user_item_modify.txt`: 현 `ItemManagerDraftController/ItemManagerSaveCommand`에 immutable `labelSizeId`·전체 `baselineColumnIds`가 없음을 명시하고 컬럼 구조 저장 활성화 전 선행 구현으로 바꿨다. 공용 save result/dirty·품목 요소 queue, 특별 항목 form transaction, stable-key table API를 소유 모듈별 선행 단계로 분리하고 기존 property-only 수정/사용자 항목 CRUD vertical slice 뒤 품목 baseline·journal·교차 잠금·content 보충을 완료한 같은 배포 단위에서 add/delete/reorder를 활성화하도록 구현 순서와 완료 조건을 정리했다. diagnostics 오류 0건이다.
- 독립 재검토 보완 `doc/user_item_modify.txt`: 사용자 항목 UI의 전체 목록 uniqueness 표현을 신규/keyword 변경 행의 새 위반 검증으로 좁히고, 품목 SQL 절차에 item parent trigger signature 재확인과 `none/validatedLegacy/unsupported`별 보조행 생성/차단 분기를 추가했다. trigger fingerprint canonical tuple에 `sys.sql_modules.uses_quoted_identifier/uses_ansi_nulls`를 포함하고 identifier case·delimiter·escape spelling을 보존하도록 고정했다. 후보 add/delete/reorder handler는 구조 안전조건 전 비활성화하고 property-only도 DAO/fake callback 선행 검증만 허용하며, 품목 baseline·journal·교차 잠금 완료 뒤 실제 컬럼 저장 UI를 함께 활성화하도록 순서를 정정했다. 본문·DAO 테스트·완료 조건 보완 직후 diagnostics 오류 0건이다.
- 승인 재검토 보완 `doc/user_item_modify.txt`: 현 레거시 v1/v2 trigger 원본에 맞춰 `ItemSaveSchemaCapabilities`를 regular column, v2 element, v1 barcode/image source-child pair의 실제 key·빈 기본값·적용성·정확히 한 행 final-state 계약으로 정의했다. transaction 전에 확정한 expected trigger metadata 행 집합을 부모/application lock 뒤 현재 집합과 양방향 `EXCEPT`로 비교하고 불일치 시 DML 전 schema-changed 오류·cache 무효화·working copy 유지·자동 재시도 금지로 닫았다. 사용자 항목 사전은 예약 keyword 자체를 허용하고 실제 일반 사용 항목 추가에서만 차단하는 기존 정책으로 통합 회귀 문구를 정정했다. DAO/통합 테스트 연결과 각 patch 직후 diagnostics 오류 0건이다.
- 최종 경합 보완 `doc/user_item_modify.txt`: metadata exact-set 비교와 첫 insert 사이 trigger DDL TOCTOU를 막기 위해 신규 품목/컬럼 transaction만 `label-size 부모 → 공용 application lock → trigger parent TABLOCKX,HOLDLOCK schema guard → metadata 비교 → 자식 DML` 순서로 잠그도록 했다. table X lock이 DDL의 `Sch-M`을 transaction 종료까지 대기시키므로 검증한 definition/enable 상태가 trigger 실행 중 바뀌지 않는다. property-only에는 적용하지 않으며 DDL 경쟁 fixture, deadlock/부분행 없음, 완료 조건까지 연결했다. 편집 직후 diagnostics 오류 0건이다.
- 검증 예정: 두 문서 diagnostics, 구형 trigger 필수·전체 keyword 소급 검증·현재 controller 보유 표현 검색, 독립 재검토, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`, 관련 diff/status를 수행한다. 문서만 변경하므로 Flutter 테스트·빌드·배포는 실행하지 않는다.
- 최종 검증 실행 예정: 독립 최종 재검토 두 건은 치명·높음·중간 finding 없이 승인했고, 두 문서 diagnostics 오류 0건 및 구형 현재-controller 보유·전체 keyword 소급·canonical header 표현 0건을 확인했다. 이제 `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`, 두 파일 제한 diff와 `git status --short`를 실행한다.
- 최종 검증 완료: 두 문서 diagnostics 오류 0건, 구형 현재-controller 보유·전체 keyword 소급·canonical header 표현 0건이며 새 trigger 3상태·SET metadata/identifier 보존·DDL schema guard·비소급 keyword·단계별 활성화 계약의 본문/DAO 테스트/통합 회귀/완료 조건 연결을 확인했다. 독립 최종 재검토 두 건은 치명·높음·중간 finding 없이 승인했고 `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`를 통과했다. 문서만 변경해 Flutter 테스트·빌드·배포와 임시 산출물/캐시는 생성하지 않았다.
...
- 작업지시서 로컬 커밋 완료: `e72cd53` (`문서: 항목 편집기 저장 정책 재정리`). `doc/user_item_modify.txt`만 포함했다. 마지막 stage/commit 대상은 이 해시와 완료 상태를 기록한 `SESSION_HANDOFF.md` 단독이며, 기존 unrelated `.vscode/settings.json`, `lib/core/app.dart`는 제외하고 원격 push는 수행하지 않는다.
...
- 수정 예정 `doc/user_item_modify.txt`: 기존 `DbClient.transaction()`을 유일한 transaction 소유자로 명시하고 DAO 내부 transaction 중첩을 제거한다. 품목 draft의 dirty/busy/force-reload 상태에서 진입·저장을 차단하고, 저장 성공 후 상위 강제 재조회 경계로 품목 draft/controller와 공용라벨 cache를 함께 갱신한다. 일반 pre-commit 실패와 commit 결과 미확정을 구분해 후자만 재조회 전 재저장을 차단한다. 미지원 raw 보존 및 고정 분류 order 필드 요구는 사용자 결정에 맞게 제거한다. 미검증.
- 편집 완료 `doc/user_item_modify.txt`: 현재 `TColumn.fromMap()` fallback 허용, 고정 분류 DB 반환 순서 유지, 기존 `DbClient.transaction()` 단독 소유와 DAO 내부 transaction 중첩 금지, 품목 draft dirty/busy/force-reload 진입·저장 차단, 상위 강제 재조회에 의한 cache/controller 갱신, `DbCommitOutcomeUnknown`의 재조회 전 동일 command 재저장 차단을 본문·테스트·완료 조건에 병합했다. 편집 직후 문서 diagnostics 오류 0건이다.
...
- 수정 예정 `doc/user_item_modify.txt`: at-most-once commit authorization과 unknown recovery, `StrictControlFrameV1`, P0 baseline artifact/producer 경로, capability 차단 정책, aggregate logout idle state, pending candidate render/swap, workbook role별 runtime, async login attach, shutdown source별 retirement, typed workbook save 결과, 제조일자 기준 날짜 offset, 전체 barcode wire enum/raw policy, platform target/provenance, writer/requirement trace manifest와 span 단위를 모델·본문·테스트·16.5·17·19장에 반영한다. 미검증.
- 검증 예정: 두 문서 diagnostics, exactly-once CAS/단수 runtime/idle 없는 logout/모호한 barcode 추가 형식/제조일자 없는 offset/통합 production gate 등 폐기 표현 검색, 새 계약 연결 검색, UI·DB·output·milestone 독립 재검토, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`. 문서 변경만 수행하므로 Flutter 테스트·빌드·배포는 실행하지 않는다.
- 기존 unrelated dirty `.vscode/settings.json`, `lib/core/app.dart`는 수정·stage 대상에서 제외하고 원격 push와 배포 작업은 수행하지 않는다.
- 핵심 모델 편집 완료 `doc/user_item_modify.txt`: commit authorization CAS를 native commit의 선형화점이 아닌 최대 1회 진입 허가로 교정하고 consume 뒤 `commitCompleted` 증거 전 crash window를 `commitOutcomeUnknown`으로 닫았다. `StrictControlFrameV1` request/response union과 correlation/replay/EOF 규칙을 추가했다. `FortuneSheetRuntimeV1`을 workbook role별 identity로 확장하고 typed save result/dirty baseline, coordinator-only pending candidate render token, workflow 0개·복수 개의 `LogoutWorkflowAggregateV1`, async `LoginContextAttachResultV1`, source별 `ShutdownSourceV1`을 추가했다. 유통기한은 유효 제조일자 우선 기준으로 교정하고 persisted barcode 10종 closed wire enum과 trim/check-digit/padding 없는 원문 보존 정책을 확정했다. 편집 직후 diagnostics 오류 0건이며 gate·테스트·완료조건 연결은 진행 중이다.
- 편집 완료 `doc/user_item_modify.txt`: `ArchitectureBaselineManifestV1`/`RequirementTraceManifestV1`과 실제 checked-in P-1 artifact tree·validator를 P0 entry 전에 요구하고, DB capability 미충족은 reason 기반으로 DB-dependent 기능만 fail-closed하도록 했다. `FeatureReleaseCapabilityV1`은 Windows 10/11 x64 ODBC worker를 V1 필수 target으로 우선 판정하고 후속 macOS/Android/iOS 결과를 독립시켰다. 새 UI/session/output/DB 계약을 순수·widget·integration test, 16.5 gate, P-1/P0~P3 milestone, 17장 구현 순서와 19장 완료 조건까지 연결했다.
- 잔여 계약 편집 완료 `doc/user_item_modify.txt`: `LegacyOutputTraceSpanV1`의 UTF-16 code-unit half-open span/schema, native wrapper 소유 `NativeCancelHandleStateV1`의 handle generation·execute/cancel/discard·close CAS, production strict dispatcher와 동일 call graph의 `StrictRecoveryHarnessBindingV1` 및 `strictNativeRecoveryTestAction`/`P2 post-bind integration`을 모델·테스트·gate·완료 조건에 연결했다. 완료 조건은 `LogoutWorkflowAggregateV1`, pending candidate render-before-authoritative-CAS, async login/source별 shutdown, P-1 baseline, 제조일자 우선/barcode raw, Windows 우선 capability 기준으로 통일했다. 편집 직후 두 문서 diagnostics 오류 0건이고 핵심 새 계약 27개 연결 위치를 확인했다.
- 최종 검증 실행 예정: 두 문서 diagnostics, 폐기 CAS 선형화점/단수 runtime/통합 4-platform gate/모호한 trace span/iOS cancel/dispatcher harness 표현 검색, 새 계약 연결 검색, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`, 관련 diff·상태를 확인한다. 이어 UI/session, strict DB/recovery, legacy output, milestone/release 실행성 네 관점 독립 재검토를 수행한다. 문서만 변경했으므로 Flutter 테스트·빌드·배포는 실행하지 않는다.
- 사용자 추가 결정: 제조일자는 레거시처럼 유효한 8자리 또는 14자리 `yyyyMMddHHmmss`를 허용하고 offset 기준은 앞 8자리로 한다. persisted barcode 형식은 `EAN13/CODE128/I2OF5/DataMatrix/CODE39/QRCode/MicroQRCode/UPC-A/CODE93/ISBN` 레거시 10종 exact raw 표만 지원하고 EAN8은 V1에서 차단한다. DB payload 원문은 불변으로 보존하되 `RICH_USE_BARCODE_CHECKDIGIT`가 켜진 실제 output projection에서만 레거시 check digit 교정·형식별 padding을 재현한다.
- 독립 재검토 보완 완료 `doc/user_item_modify.txt`: candidate renderReady 뒤 CAS reject의 candidate detach/old reattach, workflow registry registration-vs-retirement intent 단일 CAS, `ActiveLoginContextOwnershipV1` startup probe, role/epoch/attempt/version-bound workbook save baseline을 닫았다. commit authorization은 `commitDispatchIntentDurable` sequence/digest까지 token에 결합하고 consume 전/후 fault를 분리했다. cancel ACK 유실은 logical quarantine와 safe-free evidence로 닫고 P2 harness/post-bind 결과를 동일 binding snapshot에 묶었다. P-1 canonical artifact/schema 유한 목록, requirement planned→implemented→verified, capability target selector/dependency와 reason별 affected action mapping 및 target별 Windows 우선 gate를 추가했다.
- 최종 검증 완료: 두 문서 diagnostics 오류 0건, 폐기 8자리-only/EAN8 지원 열거/raw check-digit 0건/CAS 후 commit 0건/swap-before-render/로그인 선행 startup/모든 플랫폼 development 묶음 표현 0건이다. 새 핵심 계약은 22곳 이상 연결됐고 `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`를 마지막 편집 후 재통과했다. UI/session, strict DB/recovery, legacy output, milestone/release 네 관점 독립 재검토에서 치명·높음·중간 finding 0건이다. 문서만 변경해 Flutter 테스트·빌드·배포와 임시 산출물/캐시 생성은 수행하지 않았다.
- stage/commit 대상: 먼저 `doc/user_item_modify.txt`만 24차 작업지시서 커밋으로 포함하고, 그 commit hash를 기록한 `SESSION_HANDOFF.md`를 후속 인수인계 커밋으로 포함한다. 기존 unrelated `.vscode/settings.json`, `lib/core/app.dart`는 제외하고 원격 push는 수행하지 않는다.
- 작업지시서 로컬 커밋 완료: `4017c3d 문서: 24차 항목 편집기 구현 계약 정합화` (`doc/user_item_modify.txt` 단독, 68 insertions/37 deletions). 마지막 stage/commit 대상은 `SESSION_HANDOFF.md` 단독이며 unrelated 파일과 원격 push는 제외한다.
...
- 수정 예정 `doc/user_item_modify.txt`: `ResultContractValidatedEvidenceV1` pre-commit control frame과 digest-bound commit authorization, `GateDefinitionDigestInputV1`, durable block disposition 분리, canonical recovery phase 폐쇄 enum, reconciliation 총함수, 레거시 날짜 방향 분기, barcode symbology 불변, 독립 result-set collector 우선 순서, save/logout phase matrix, output clock digest key, checked-in DB-Lib fixture source와 field별 Natrium encoder를 모델·본문·테스트·16.5·17장·19장에 반영한다. 미검증.
- 검증 예정: 두 문서 diagnostics, 옛 `releaseDurableMutationBlock=false` 역전 의미/result-frame-only validation evidence/collector 후순위/일반 optional-sign 날짜/format 자동 fallback/모호한 stable identity 표현 검색, 새 계약 연결 검색, UI·DB·출력·문서 정합성 독립 재검토, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`. 문서 변경만 수행하므로 Flutter 테스트·빌드·배포는 실행하지 않는다.
- 기존 unrelated dirty `.vscode/settings.json`, `lib/core/app.dart`는 수정·stage 대상에서 제외하고 원격 push와 배포 작업은 수행하지 않는다.
- 편집 완료 `doc/user_item_modify.txt`: output clock single-flight key를 `(outputSessionId, stableDatabaseIdentityDigest, identityCanonicalizationVersion, connectionEpoch)`로 폐쇄했다. `ResultContractValidatedEvidenceV1` control frame과 digest-bound single-use commit authorization을 추가하고 ACK/result frame과 pre-commit 승인 증거를 분리했다. durable mutation block은 `durableBlockDisposition`으로 active block과 evidence artifact 수명을 분리했으며 canonical handoff를 정식 workflow state로 만들었다. commit-unknown 판정을 desired-only/original-only/both/neither/mixed 총함수로 닫고 no-op은 dispatch 전에 종료한다. 사용자 결정대로 commit intent 이후 workflow terminal 전 logout은 `saveBusyCommitPhase`로 차단한다. 편집 직후 diagnostics 오류 0건이며 옛 durable release 표현 2곳 정리 중이다.
- 편집 완료 `doc/user_item_modify.txt`: `GateDefinitionDigestInputV1`을 추가해 SQL/DDL, command, timeout/cancel, assertion, result schema와 expiry 의미 변경이 해당 pair result를 만료하게 하고 manifest/result를 여섯 binding으로 통일했다. DB-Lib P0 entry는 공용 C fixture source/source digest/artifact decoder 실제 checked-in을 요구하고 플랫폼별 target/binary/result만 planned로 허용한다. strict DB 순서는 독립 result-set/complete-drain에서 marker/contract/evidence/authorization으로 진행하도록 교정했다. diagnostics 오류 0건이다.
- 편집 완료 `doc/user_item_modify.txt`: `LegacyValidDateOffsetV1`의 raw-first-character negative branch, field-aware `LegacyNatriumUrlEncoderV1`, 사용자 결정의 `BarcodeSymbologyPolicyV1`을 모델·12.5·순수 테스트·16.5·구현 순서·19장에 연결했다. EAN/UPC/ITF 실패 시 Code128/QR 자동 변경을 금지하고 동일 symbology bitmap 또는 typed error만 허용한다. Natrium은 item/producer/amount만 레거시 ASCII 치환 후 UTF-8 non-ASCII byte만 percent encode하며 일반 URI component encoder를 금지한다. diagnostics 오류 0건이다.
- 중간 검증 결과: 두 문서 diagnostics 오류 0건, 옛 clock key/다섯 binding/planned fixture source/collector 후순위/date parser 단독 규범/자동 symbology fallback 표현 0건이다. 새 evidence/digest/disposition/reconciliation/logout/output 계약 40개 연결 위치를 확인했다. 당시 확인한 초장문 11.1의 구형 tuple은 아래 후속 보완에서 원문과 역사적 정정 문단까지 제거했으며 authoritative 모델·테스트·gate·완료조건은 모두 `durableBlockDisposition`만 사용한다. 독립 UI·DB·출력·문서 실행성 재검토와 `git diff --check`는 미실행이다.
- 독립 재검토 결과: UI/문서 관점에서 초장문 11.1의 폐기 boolean 원문 자체 제거, canonical handoff state의 폐쇄 목록 직접 포함, `SaveWorkflowPhaseV1`과 logout terminal 집합이 필요하다. DB/gate 관점에서 17·19장의 네 digest DTO 잔여를 다섯 DTO로 통일하고 DB-Lib source path/digest/decoder 음성 fixture, evidence ordinal/publish sequence의 replay 방지와 atomic authorization consume을 보강해야 한다. 출력 관점에서 Natrium sodium/comparison `_ttoi`·rate truncate·CP949 strict profile, hybrid EZPL clear call graph gate, barcode result union schema와 safety improvement 표시가 필요하다. 보완 후 네 관점 독립 승인을 재실행한다.
- 후속 보완 편집 완료 `doc/user_item_modify.txt`: 초장문 11.1의 구형 `releaseDurableMutationBlock` tuple과 이를 뒤집던 역사적 정정 문단을 원문에서 제거했다. 해당 문자열 검색 0건, 편집 직후 문서 diagnostics 오류 0건이다. 나머지 독립 재검토 finding은 미검증 상태로 계속 보완한다.
- UI/recovery 후속 보완 완료 `doc/user_item_modify.txt`: `ReconciliationIndeterminateReasonV1`, `SaveWorkflowPhaseV1`, `LogoutTerminalWorkflowStateSetV1`을 추가해 reconciliation reason, freeze 전/후 phase projection과 일반 logout 허용 terminal을 폐쇄했다. `authoritativeCandidateSwappedAndOldGenerationRetired`를 실제 `workflowStateSet` 폐쇄 목록에 직접 포함했다. 편집 직후 diagnostics 오류 0건이고 canonical state 목록 삽입 검색 1건이다.
- DB/gate 후속 보완 완료 `doc/user_item_modify.txt`: commit authorization을 evidence ordinal·parent durable publish sequence·digest tuple에 결합하고 commit 직전 `consumeCommitAuthorization(expectedTokenVersion)` 원자 CAS가 한 번 성공한 호출만 native commit에 진입하도록 모델·테스트·16.5 gate를 통일했다. 17·19장의 digest 입력을 다섯 DTO의 정확한 타입 목록으로 수정하고 `gateDefinitionDigest` 여섯 binding/expiry를 완료조건에 포함했다. DB-Lib fixture source missing/root escape·symlink/junction escape/digest mismatch/decoder missing·schema version mismatch 음성 테스트와 P0 entry 거부 조건을 추가했다. 각 편집 직후 diagnostics 오류 0건이다.
- 출력 후속 보완 완료 `doc/user_item_modify.txt`: `LegacyNatriumEncodingProfileV1`과 sodium/comparison `_ttoi`·comparison 0의 빈 `nn` 성공 분기·native division rate truncate·`IntToString`·comparison raw 비삽입을 모델/12.5/golden/16.5/17/19장에 연결했다. CP949 malformed/unmappable/embedded NUL과 replacement 금지를 고정했다. `RequestedBarcodeSymbologyV1`, versioned `BarcodePayloadResultV1`/`BarcodeRendererResultV1` union을 추가하고 logical invalid는 renderer 전 typed error, valid payload의 native wire unrepresentable만 동일 symbology bitmap fallback으로 분리했다. 자동 symbology 변경 금지는 parity, 동일 형식 fallback 제한은 안전성 개선으로 구분했다. hybrid EZPL의 `_addHybridSheetBarcodes`/`_canRenderNativeBarcode`/`_clearNativeBarcodeFallbackAreas`가 per-object `nativeWire` 검증 전 clear하지 못하도록 구조 gate를 추가했다. 편집 직후 diagnostics 오류 0건이고 구 decimal-text/ANSI/invalid→bitmap 표현 검색 0건이다.
- 레거시 원문 반증 검사 및 교정 `doc/user_item_modify.txt`: `.tmp/LabelManager/LabelManagerLib/QRCodeNatriumInfo.cpp`와 `NatriumCalculator.cpp`를 다시 확인한 결과 comparison 0은 오류가 아니라 legacy success URL의 `nn` 값만 빈 문자열로 두는 분기였다. 모델/12.5/golden/19장 4곳을 `_ttoi` sodium/comparison → URL `n=IntToString(sodiumInt)` → nonzero comparison의 C++ truncation-toward-zero rate 또는 zero comparison의 빈 `nn`으로 교정했다. 잘못 추가됐던 `invalidComparisonZero`/comparison 0 차단 검색 0건, 교정 직후 diagnostics 오류 0건이다.
- 최종 기계 검증 실행 예정: 두 문서 diagnostics, 폐기 계약/새 심볼 연결 검색, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`, 관련 두 문서 diff·통계와 working tree 상태를 확인한다. 이후 UI/recovery·DB/gate·legacy output·문서 실행성 네 관점 독립 재검토를 수행한다. 문서 변경만이므로 Flutter 테스트·빌드·배포는 실행하지 않는다.
- 최종 기계 검증 중간 결과: 두 문서 diagnostics 오류 0건, 작업지시서의 폐기 계약 검색은 commit 후 result frame을 pre-commit evidence로 쓰는 경로를 0건으로 요구하는 의도된 금지 문장 1건뿐이다. 새 recovery/authorization/gate/Natrium/barcode/hybrid 계약은 26개 위치에 연결됐다. `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md` 통과, 관련 변경량은 2개 문서 86 insertions/38 deletions다. 기존 unrelated dirty `.vscode/settings.json`, `lib/core/app.dart`는 stage/commit에서 제외한다.
- 독립 재검토 2차 결과: 치명 finding은 없었다. UI/recovery는 recovery-blocked terminal의 retirement 여부와 구 durable boolean 의미, DB/gate는 worker evidence/parent publish sequence 소유 시점·CAS의 17/19장 누락·DB-Lib 반대 단위 음성 검출·digest 계산 순서, output은 rate의 32-bit 범위 밖 cast와 CP949 strict 정책의 parity/safety 구분을 보완해야 했다.
- 사용자 추가 결정: Natrium `rateDouble`이 signed 32-bit int 범위 밖이거나 non-finite이면 레거시 undefined C++ cast를 재현하지 않고 typed `natriumRateOutOfRange`로 차단한다.
...
- 사용자 요청: 21차 작업지시서를 레거시/현 프로젝트에 다시 대조해 확인한 FortuneSheet runtime ownership, live/durable mutation barrier, logout recovery probe, DB-Lib ABI length, gate cardinality/digest, P0/P2 recovery, Godex wire, server clock, duplicate keyword와 전환 gate 문제의 권장안을 `doc/user_item_modify.txt`에 병합한다.
- 사용자 결정: FortuneSheet workbook/mutation queue/undo/editor 상태는 Flutter `State` 밖의 root-owned runtime으로 추출한다. duplicate keyword는 레거시 순서 승자를 유지해 QR/user-defined/barcode-link는 첫 ordered column이 token을 소비하고 Natrium은 마지막 ordered match가 승리한다. server date snapshot 취득 실패는 자동 fallback하지 않고 사용자 확인을 받은 해당 출력에만 PC 날짜 snapshot을 사용한다. worker artifact 변경은 root digest와 target entry digest를 분리해 해당 target gate 결과만 만료한다.
- 수정 예정 `doc/user_item_modify.txt`: `FortuneSheetRuntime` owner와 breaking API migration, `LiveWorkbookMutationBarrier`/`DurableLabelSizeMutationBlock` 분리, original-observed reload/swap, logout completion probe evidence, close timeout cancellation, DB-Lib ABI fixture 우선 계약, `(gateId,targetId)` identity와 digest input envelope, P0/P2 strict 실행 조건, affected count marker, Godex representability fallback, output clock snapshot/사용자 승인 fallback, duplicate keyword parity와 end-to-end resolver 전환 gate를 모델·본문·테스트·16.5·17장·19장에 반영한다. 미검증.
- 검증 예정: 두 문서 diagnostics, 옛 root-controller/cold live barrier/code-unit 확정/single target gate/self-digest/global artifact expiry/duplicate keyword 차단/무조건 Godex `&G`/동기 server clock 표현 검색, 새 계약 연결 검색, UI·DB·출력·문서 정합성 독립 재검토, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`. 문서 변경만 수행하므로 Flutter 테스트·빌드·배포는 실행하지 않는다.
- 기존 unrelated dirty `.vscode/settings.json`, `lib/core/app.dart`는 수정·stage 대상에서 제외하고 원격 push와 배포 작업은 수행하지 않는다.
- UI/runtime/recovery 편집 완료 `doc/user_item_modify.txt`: root-owned `FortuneSheetRuntimeV1`과 typed mutator migration, process-local `LiveWorkbookMutationBarrier`/crash-safe `DurableLabelSizeMutationBlock`, original-observed candidate swap, `LogoutCompletionProbeV1`, `CloseAttemptV1`, guarded app pop/owned route dismiss를 모델·본문·테스트·P2/16.5·구현 순서·19장에 연결했다. diagnostics 오류 0건, 새 계약 30개 위치를 확인했다.
- 예외 기록: UI 편집 subagent가 요청 범위를 넘어 `doc/user_item_modify.txt` 단독 로컬 커밋 `3467f71` (`문서: 22차 UI 런타임 복구 계약 반영`)을 생성했다. 커밋 범위는 작업지시서 1개뿐이고 unrelated 변경은 포함하지 않았다. 커밋을 되돌리지 않고 이후 DB·출력·최종 정합화 변경을 별도 커밋으로 이어간다.
- UI 잔여 정합화: 기존 session resource 본문의 `controller/journal/listener` 표현을 `runtime/journal/listener`로 교체하고 mode별 release field와 pre-commit durable terminal 의미가 독립 재검토에서 구현 가능하게 닫혔는지 최종 확인한다.
- DB ABI/gate 편집 완료 `doc/user_item_modify.txt`: FreeTDS header만으로 NVARCHAR 길이 단위를 확정하지 않고 exact bundle/ABI/API/type native fixture가 `DbLibRpcLengthCapabilityV1`을 결정하게 했다. gate identity를 `(gateId,targetId)` pair로 바꾸고 self-digest를 제외한 네 digest input DTO와 target별 `workerArtifactEntryDigest` 만료를 추가했다. P0-only production strict dispatch를 금지하고 machine-readable manifest를 P0 entry source of truth로 요구했으며 aggregate affected count를 diagnostic-only로 낮췄다. 옛 `byteLength / 2`/전역 worker digest/P0 production strict/affected-count 권한 표현 검색 0건, 새 계약 15개 위치, diagnostics 오류 0건, `git diff --check -- doc/user_item_modify.txt` 통과다.
- 출력 edge 편집 완료 `doc/user_item_modify.txt`: duplicate keyword는 레거시 순서대로 QR/user-defined/barcode-link의 첫 ordered column과 Natrium의 마지막 ordered column이 승리하게 했다. `OutputClockSnapshotV1`은 SQL Server 날짜를 session당 1회 취득하고 실패 시 같은 output session의 blocking 사용자 승인에만 local date snapshot을 허용한다. `GodexBarcodeWireResultV1`은 representable native wire 확정 뒤에만 bitmap fallback 영역을 지우며 literal escape/비ASCII/control/FNC1 표현 불가 payload는 fallback을 유지한다. 현 `ItemCodeDataResolver` 직접 token/date/Natrium 구현 제거와 전 출력 경로 동일 logical payload E2E gate를 추가했다. 옛 duplicate keyword 차단/자동 client fallback/무조건 Godex native 표현 검색 0건, 새 계약 18개 위치, diagnostics 오류 0건, `git diff --check -- doc/user_item_modify.txt` 통과다.
- 독립 재검토 1차 결과: UI에서 pre-commit durable block terminal/release, transition activation mode 총함수와 cold original-observed fixture, close cancellation owner가 남았다. DB에서 worker normalized row digest input과 approved root snapshot membership이 남았고 DB-Lib capability/affected-count production gate 연결이 부족했다. 출력에서 parser duplicate 차단·18장 신규/기존 duplicate 정책, preview/print output session 범위, clock fallback single-flight와 Godex literal ampersand/command profile 기대값이 남았다. 문서 정합성에서는 DB dispatch의 `둘 다 없으면 거부` 논리 오류와 명명 gate 누락이 확인됐다.
- 후속 정책: 기존 persisted duplicate keyword는 grandfathered 허용하고 레거시 순서 승자를 적용하되 신규 추가/rename으로 duplicate keyword를 만드는 것은 validation에서 차단한다. preview refresh와 실제 print command는 별도 output session이며 각 command 내부 downstream만 같은 payload/clock을 공유한다. preview 그대로 인쇄는 payload/item/workbook generation pin이 모두 일치할 때만 허용하고 아니면 새 print session에서 재생성한다. Godex 리터럴 `&`, `&&`, `&A`~`&G`, trailing `&`는 명령 profile별 실제 장비 escape 증거가 없으면 모두 unrepresentable bitmap fallback으로 고정하고 논리 FNC1의 `&G`만 검증된 command/subset profile에서 허용한다.
- 최종 승인 재검토 잔여: `CloseAttemptV1` 정상 완료와 취소 경합의 단일 CAS, preview clock-bound payload의 새 print session 재사용 모순, clock acquisition join Future의 취소 terminal, literal ampersand 일반 규칙, P0 entry에서 실제 target 전 DB-Lib 검증 capability를 요구하는 순환이 남았다. 해소 방향은 preview payload를 print에 재사용하지 않고 실제 print가 새 session/clock/payload를 항상 재생성하며, P0 entry에는 planned DB-Lib fixture descriptor만 요구하고 target 생성 뒤 development-complete에서 검증 capability manifest를 생성하는 것으로 확정한다.
- 최종 보완 완료 `doc/user_item_modify.txt`: 정상 close callback과 detach/shutdown cancellation이 동일 attempt ID/token/state의 단일 CAS를 경쟁하게 했다. preview와 print의 clock-bound payload 재사용을 제거하고 command별 새 session/clock/payload를 생성하며 joined clock 취소는 `OutputClockCancelled(reason)`로 exactly-once 종료한다. Godex raw logical data의 모든 literal U+0026은 V1에서 fallback하고 adapter가 생성한 logical FNC1 `&G`만 검증 profile에서 허용한다. P0 entry는 `DbLibRpcLengthFixturePlanV1`만 요구하고 actual target fixture와 `DbLibRpcLengthCapabilityV1` 생성은 development-complete로 이동했다.
- 최종 검증 결과: 두 문서 diagnostics 오류 0건, 옛 단일 barrier/전역 worker digest/code-unit 단정/preview payload 재사용/P0 capability 순환/duplicate output 차단 규범 0건, 새 runtime·barrier·DB ABI/digest·output clock/Godex 계약 44개 이상 연결 위치 확인, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md` 통과다.
- 최종 독립 승인: UI/runtime/recovery, DB substrate/release, legacy/GS1 output, 문서 전체 실행성 관점에서 치명적·높음·중간 finding 0건으로 승인됐다. 문서만 변경했으므로 Flutter 테스트·빌드·설치 패키징은 실행하지 않았고 임시 산출물/캐시는 생성하지 않았다.
- stage/commit 대상: 조기 UI 계약 커밋 `3467f71` 이후 남은 DB·출력·최종 정합화 변경은 `doc/user_item_modify.txt` 단독으로 커밋한 뒤 `SESSION_HANDOFF.md`를 단독 커밋한다. 기존 unrelated `.vscode/settings.json`, `lib/core/app.dart`는 제외하고 원격 push는 수행하지 않는다.
- 작업지시서 커밋 완료: UI/runtime/recovery 계약은 `3467f71` (`문서: 22차 UI 런타임 복구 계약 반영`), DB ABI/gate·출력 edge·최종 정합화는 `f673462` (`라벨 항목 편집기 작업지시서 22차 권장안 정합화`)로 각각 `doc/user_item_modify.txt` 단독 로컬 커밋했다.
- 마지막 stage/commit 대상은 `SESSION_HANDOFF.md` 단독이다. 원격 push와 배포 작업은 수행하지 않는다.

### 진행 중 (2026-07-13): 라벨 항목 편집기 작업지시서 21차 검토 권장안 병합

- 사용자 요청: 20차 확정본을 레거시/현 프로젝트와 다시 대조해 확인한 DB collector/NVARCHAR, GS1 encoder parity, overlay/controller lifecycle, pending request identity, recovery/barrier/milestone 모델 문제의 권장안을 `doc/user_item_modify.txt`에 병합한다.
- 사용자 결정: legacy-permissive 실제 출력의 예약 품목 token은 `#ITEMNAME` 하나만 사용한다. 같은 process의 logout은 로그인 화면 전환 전에 root session/recovery ownership을 durable retirement한다. `WorkbookSaveAuthorizationBarrier`는 authoritative candidate swap과 old generation retirement가 확정된 폐쇄 전이에서만 해제한다.
- 수정 예정 `doc/user_item_modify.txt`: root close dispatcher, pending request identity, FortuneSheet 단일 attachment/mutation gate, barrier release transition과 logout retirement, result-set별 strict collector, DB-Lib NVARCHAR 길이 단위, item writer lock resolver, platform strict boundary, phase-aware isolate termination, encoder별 GS1 FNC1, raw keyword/ITEMNAME/date/natrium/RTF parity, evidence/target 모델, P3/cold API/connection test 순서를 본문·모델·테스트·구현 순서·완료 조건에 반영한다. 미검증.
- 검증 예정: 두 문서 diagnostics, 옛 `#품목` output alias/공통 `0x1D` wire/모호한 barrier terminal/request identity/cold-only initial activation/first-schema collector 표현 검색, 새 계약 연결 검색, 독립 UI·DB·GS1·문서 정합성 재검토, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`. 문서 계약만 변경하므로 Flutter 테스트와 배포 빌드는 실행하지 않는다.
- 기존 unrelated dirty `.vscode/settings.json`, `lib/core/app.dart`는 수정·stage 대상에서 제외하며 원격 push와 배포 작업은 수행하지 않는다.
- 편집 완료 `doc/user_item_modify.txt`: root `BlockingOverlayStack.dispatchClose()`의 `handled | consumedBusy | unhandledBecauseEmpty` 결과와 exit 위임 조건, pending request의 requestId/command/target 의미 identity와 initiator-only continuation, FortuneSheet 단일 attachment lease/공용 mutation gate, authoritative swap+old retirement 전용 barrier 해제를 본문·모델에 반영했다. 편집 직후 diagnostics 오류 0건이다.
- 편집 완료 `doc/user_item_modify.txt`: result-set별 `DbResultSetV1` strict collector, `DbLibRpcLengthV1`, evidence/target matrix 명시 모델, platform artifact packaging, item writer의 caller-supplied labelSizeId/lock 내부 소유권 검증과 phase-aware `StrictWorkerTerminationClassifierV1` 계약을 추가했다. 편집 직후 diagnostics 오류 0건이다.
- 편집 완료 `doc/user_item_modify.txt`: 사용자 확정대로 실제 출력 예약 token을 `#ITEMNAME` 하나로 제한하고 raw keyword trim/hash 정규화를 금지했다. 나트륨 record dependency, legacy date/server-clock 변환, RTF 1글자 padding 분리와 논리 FNC1을 Zint `0x1D`/Godex EZPL `&G`로 인코딩하는 adapter별 wire·scan fixture를 추가했다. 편집 직후 diagnostics 오류 0건이다.
- 정합화 완료 `doc/user_item_modify.txt`: pending request semantic identity/initiator-only continuation, root close dispatcher 결과, FortuneSheet 단일 attachment/mutation gate와 exact barrier 해제, result-set별 collector/DB-Lib 길이/phase-aware worker 종료, `#ITEMNAME` only·raw keyword·natrium/date/RTF·encoder별 FNC1 계약을 pure/widget/DAO/integration test와 19장 완료 조건에 연결했다. 각 substantive 편집 직후 diagnostics 오류 0건이다.
- 편집 완료 `doc/user_item_modify.txt`: 같은 process logout은 로그인 화면 전환 전에 `LogoutSessionRetirementV1`으로 현재 session/controller/journal ownership과 authoritative reference를 durable retirement하고, 실패 시 로그인 화면 전환과 새 login context attach를 차단하도록 모델·root lifecycle·구현 순서를 통일했다. unresolved strict operation evidence/resource barrier는 identity namespace에 보존한다. root activation API는 `activateCandidate(LabelSizeActivationMode, candidate)`로 warm CAS와 cold recovery initial activation을 분리했다. 편집 직후 diagnostics 오류 0건이다.
- 정합화 완료 `doc/user_item_modify.txt`: P3 entry는 `reservedFutureWriter` 계약만 확인하고 실제 command/DAO owner 생성 직후 `existingWriter`로 승격하도록 milestone 순환을 제거했다. 정상 연결 테스트는 `transportConnecting`을 포함하도록 수정했다. 최종 stale 검색·전체 diagnostics·diff check·독립 재검토는 미실행이다.
- 정합화 완료 `doc/user_item_modify.txt`: `NativeOperationEvidenceDescriptorV1` producer/prerequisite/digest, `RequiredTargetMatrixV1` tuple/digest, Android/iOS/macOS artifact packaging, logout phase crash/retry/재로그인, close dispatcher와 pending semantic identity를 테스트·release gate·19장 완료 조건에 직접 연결했다. 편집 직후 diagnostics 오류 0건이다.
- 최종 검증 실행 예정: 두 문서 `get_errors`, 옛 `ITEMNAME/품목`·공통 encoder `0x1D`·generic recovery terminal·warm-only activation·logout detach·P3 owner 순환·old trace union 검색, 새 UI/DB/GS1/logout 계약 연결 검색, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`, 변경 상태/통계를 실행한다. 이후 UI·DB·GS1·문서 정합성 독립 재검토를 수행한다.
- 기계적 최종 검증 결과: 두 문서 diagnostics 오류 0건, stale 표현 검색 0건, 새 UI/DB/GS1/logout 핵심 계약 45개 위치 확인, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md` 통과다. 변경 통계는 두 문서 66 insertions/27 deletions이며 기존 unrelated `.vscode/settings.json`, `lib/core/app.dart`는 그대로다.
- 독립 재검토 실행 중: UI lifecycle, DB substrate, GS1/레거시 출력, 문서 내부 모델·테스트·milestone·완료 조건 정합성을 각각 검토한다. 문서 계약만 변경했으므로 Flutter 테스트와 배포 빌드는 실행하지 않는다.
- 독립 재검토 1차 결과: UI/recovery에서 cold recovery barrier release 증거, close callback disposition/stack entry token, async mutation lease epoch, logout closed phase가 부족했다. DB에서는 allowed-target release 판정식, worker artifact digest binding, item writer negative ownership fixture가 부족했다. 출력에서는 12.4 parser 재사용 잔여 문구, natrium item→column overwrite, RTF padding 분기 범위와 encoder 구현 순서가 부족했다. 문서 정합성에서는 canonical recovery row에 barrier release 필드와 logout production gate 연결이 부족했다.
- 보완 완료 `doc/user_item_modify.txt`: `BlockingCloseDisposition`과 stack-issued `BlockingOverlayEntryToken`, start-time `(ownerToken, attachmentEpoch)` mutation lease, canonical `RecoveryTransitionTableV1` row의 `transitionId/releaseWorkbookBarrier`, warm live-old retirement와 cold durable-predecessor retirement guard를 추가했다. `LogoutSessionRetirementV1`을 `intentPublished → listenersDetached → writeQueueDrained → resourcesDisposed → authoritativeReferenceCleared → completed` 폐쇄 phase로 고정하고 recovery/P2/19장 gate에 연결했다. 편집 직후 diagnostics 오류 0건이다.
- 보완 완료 `doc/user_item_modify.txt`: `RequiredGatePolicyV1.allowedTargetKinds`로 필수 gate-target 조합과 빈 coverage를 판정하고 `workerArtifactManifestDigest`/설치 hash/protocol/signing/notarization digest 변경 시 gate 결과를 만료하도록 모델·16.5·구현 순서·19장을 통일했다. item writer의 lock 전 조회, 다른 connection ownership 조회, stale context와 labelSizeId 불일치를 negative fixture로 추가했다. 편집 직후 diagnostics 오류 0건이다.
- 보완 완료 `doc/user_item_modify.txt`: 실제 출력은 parser token stream을 재사용하지 않는다는 우선 규칙, natrium item equality 뒤 persisted column-order overwrite, item/일반 plain-text fallback column만의 RTF padding, logical FNC1 encoder adapter와 wire/scan 구현 단계를 추가했다. pending 네 필드 mismatch, warm/cold barrier fault와 logout phase crash fixture를 완료 조건까지 연결했다. 편집 직후 diagnostics 오류 0건이다.
- 독립 보완 후 최종 재검증 예정: 미정의 `RecoveryTransitionDescriptorV1`, old close/mutation API, stale RTF/parser/release 표현, release/logout/barrier/output 연결을 검색하고 두 문서 diagnostics 및 `git diff --check`를 재실행한 뒤 네 관점 독립 승인 재검토를 수행한다.
- 독립 보완 후 기계 검증 결과: 두 문서 diagnostics 오류 0건, 미정의 recovery descriptor/old close·mutation API/옛 activation·logout·GS1·trace 표현 검색 0건, 새 close/lease/barrier/logout/release/output 계약 27개 핵심 위치 확인, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md` 통과다. 독립 승인 재검토를 시작한다.
- 독립 승인 재검토 2차 결과: UI/recovery는 승인됐다. DB에서 옛 `blocking=true` 필수 집합 생성식, target matrix canonical digest와 gate binding field가 남았고, 출력에서 실제 output parser 공유 원문이 남았다. 문서 정합성에서는 logout 완료형 phase를 side effect 전에 publish하는 모순과 barrier 구현 순서의 `9단계 row` 표현이 남았다.
- 최종 보완 완료 `doc/user_item_modify.txt`: 옛 gate 생성식과 실제 출력 parser 공유 원문을 직접 교체하고 메타 보정 문단을 삭제했다. `RequiredTargetMatrixV1.requiredTargetMatrixDigest`와 `GateManifestV1`/result binding을 명시하고 logout을 pending/done phase, completion probe, owner-scoped 멱등 side effect와 postcondition으로 닫았다. barrier 해제 port는 단계 번호 없이 canonical transition ID와 `releaseWorkbookBarrier=true`를 직접 참조한다. 편집 직후 diagnostics 오류 0건이다.
- digest 후속 보완 완료 `doc/user_item_modify.txt`: `CanonicalGateDigestV1`을 추가해 Unicode NFC 선검증, trim/case-fold 금지, explicit null, RFC 8785 JSON canonicalization, UTF-8, row별 ordinal 정렬 key, schema/폐쇄 field, SHA-256 64자리 lowercase hex를 고정했다. `GateManifestV1`과 gate result는 `policyDigest`/`requiredTargetMatrixDigest`/`matrixSchemaVersion`/`targetId`/`workerArtifactManifestDigest` 다섯 binding을 필수로 보존한다. 모델·16.5·구현 순서·19장을 같은 field로 통일했고 diagnostics 오류 0건이다.
- 최종 독립 승인: UI/recovery, DB substrate/release digest, GS1/legacy output, 문서 내부 정합성 네 관점 모두 치명적·높음·중간 finding 0건으로 승인됐다.
- 최종 검증 예정: 두 문서 diagnostics, stale gate/parser/logout/barrier/digest 표현 검색, 새 canonical digest/logout/output 계약 연결 검색, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`, 변경 상태와 stage 대상 확인을 실행한다. 문서 계약만 변경했으므로 Flutter 테스트·빌드·설치 패키징은 실행하지 않는다.
- 최종 검증 결과: 두 문서 diagnostics 오류 0건, stale gate/parser/logout/barrier/digest 표현 검색 0건, `CanonicalGateDigestV1`/logout pending·done/close lease/barrier/output 핵심 계약 27개 연결 위치 확인, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md` 통과다. 관련 변경은 두 문서뿐이며 기존 unrelated `.vscode/settings.json`, `lib/core/app.dart`는 그대로다.
- 임시 산출물/캐시: 생성하지 않았다. 첫 stage/commit 대상은 `doc/user_item_modify.txt` 단독이며 기존 unrelated `.vscode/settings.json`, `lib/core/app.dart`와 `SESSION_HANDOFF.md`는 첫 커밋에서 제외한다.
- 작업지시서 커밋 완료: `5362ce6` (`라벨 항목 편집기 작업지시서 21차 권장안 병합`)로 `doc/user_item_modify.txt`만 로컬 커밋했다. 원격 push와 배포 작업은 수행하지 않았다.
- 마지막 stage/commit 대상은 `SESSION_HANDOFF.md` 단독이다. 기존 unrelated `.vscode/settings.json`, `lib/core/app.dart`는 제외한다.

### 완료 (2026-07-13): 라벨 항목 편집기 작업지시서 20차 검토 권장안 병합

- 사용자 요청: 19차 반영본을 레거시/현 프로젝트와 다시 대조해 확인한 pending 행 command 소비, 속성 취소 의미, cold-start activation, 시스템 back/close 중재, result decode 책임, draft mapping key, 출력 order, 구현 순서와 GS1 출력 정책을 `doc/user_item_modify.txt`에 병합한다.
- 사용자 결정: GS1 품목 값이 AI 형식 검증에 실패해도 레거시처럼 contain 관계의 AI/token 문자열을 조립하고 persisted column order의 순차 치환 결과를 출력한다. 현 `ItemCodeDataResolver`의 값 형식 fail-closed는 신규 공용 GS1 출력 계약으로 교체하되 관계 endpoint/AI 설정 구조 검증은 유지한다.
- 수정 예정 `doc/user_item_modify.txt`: `PendingPropertyResolution` command-consumed 결과와 pending 취소 분기, warm/cold session activation, top-token 시스템 back/window close, 통합 저장 최종 확인, pre/post-commit decode 책임, `DraftMappingKeyV1`, duplicate order 정책, substrate 구현 순서, GS1 legacy-permissive 출력 계약을 본문·테스트·완료 조건에 반영한다. 미검증.
- 검증 예정: 두 문서 diagnostics, 옛 pending continuation/모호한 취소/cold expected-old 강제/GS1 fail-closed·duplicate order 모순 표현 검색, 독립 현 프로젝트·레거시 정합성 재검토, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`. 문서 계약만 변경하므로 Flutter 테스트는 실행하지 않는다.
- 기존 unrelated dirty `.vscode/settings.json`, `lib/core/app.dart`는 수정·stage 대상에서 제외하며 원격 push와 배포 작업은 수행하지 않는다.
- 편집 완료 `doc/user_item_modify.txt`: pending 신규 행 자체의 삭제 요청은 `requestConsumedByPendingRowRemoval`로 command가 완료되어 continuation을 재실행하지 않게 했다. 속성 `취소`는 pending 행 제거 확인과 일반 draft baseline 복원을 폐쇄 결과로 분리했다. 편집 직후 두 문서 diagnostics 오류 0건이다.
- 편집 완료 `doc/user_item_modify.txt`: `BlockingOverlayStack`이 Esc/제목 닫기뿐 아니라 모바일 system back·Navigator pop·desktop window close를 top-token command로 중재하도록 했다. workbook pin 뒤 DB 품목 영향과 참조 영향을 단일 최종 확인으로 합치고 freeze 뒤 추가 확인을 금지했다. session activation은 warm handoff와 cold recovery로 분리해 재시작 경로가 존재하지 않는 old generation/resources를 요구하지 않게 했다. 편집 직후 diagnostics 오류 0건이다.
- 편집 완료 `doc/user_item_modify.txt`: label/customer 공통 `DraftMappingKeyV1`을 lowercase ASCII UUID 36자와 SQL ordinal staging으로 고정하고, source key/ROW_NO/inserted ID 결합을 row-by-row 또는 승인 insert-only MERGE로만 허용했다. `DbValidatedResultPayloadV1` 뒤 DTO mapper 책임을 total projection으로 제한하고 typed result DTO/codec → complete-drain collector/native session → raw DAO 순서로 구현 단계를 재배치했다.
- 편집 완료 `doc/user_item_modify.txt`: 사용자 결정대로 `Gs1OutputPolicyV1.legacyPermissive`를 추가해 품목 값 형식 실패에도 레거시 AI/token/FNC1 조립·순차 치환 결과를 유지하고 구조 오류만 차단했다. 기존 raw 출력 order는 gap/nonpositive라도 유일하면 숫자 오름차순으로 허용하고 duplicate만 차단하며, 신규 저장 projection만 1부터 연속·유일하게 했다. ID 정렬은 duplicate order 허용 tie-break가 아니다. 새 UI/lifecycle/DB/GS1 계약을 pure/widget/DAO/integration fixture에 연결했고 편집 직후 diagnostics 오류 0건이다.
- 정합화 완료 `doc/user_item_modify.txt`: 새 close/activation/mapping/result/GS1 타입을 모델 목록과 구현 순서에 추가하고 19장 production 완료 조건까지 연결했다. 옛 opaque key/무조건 pending continuation/duplicate-order tie-break/warm-only cold recovery 표현 검색 0건, 새 핵심 계약 31개 위치, 두 문서 diagnostics 오류 0건, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md` 통과다.
- 독립 검토 실행 예정: pending command 소비와 close source, workbook confirm/freeze, warm/cold activation, mapping key/result payload, label/customer source mapping, GS1 legacy-permissive/order/trace 및 테스트·완료 조건의 상호 충돌을 현 프로젝트와 레거시 근거로 재검토한다. 검토 후 같은 검색·diagnostics·diff check를 재실행한다.
- 독립 검토 결과 보완 진행 중: UI/lifecycle에서 품목/output-context snapshot TOCTOU, pending commit 거부 결과, lease pin release 이중 소유, cold recovery의 이전 process render ACK 재사용, pending resolver single-flight와 임의 `Navigator.pop` 중재 불가능을 확인했다. DB에서는 strict session `XACT_ABORT/NOCOUNT` 누출, validated payload wire schema와 total mapper 범위, mapping contract 구현 단계 순서를 확인했다. GS1에서는 context별 예약 token pass, contain 순서 source 부재, rename과 endpoint dependency 혼합, gap/nonpositive order 과잉 차단, 다중 occurrence trace 좌표와 현 테스트 반대 기대를 확인했다.
- 독립 보완 후 재검토 결과: 치명적 문제는 없고, DB transaction에서 재관측 가능한 item/content projection과 in-memory workbook observation 혼합, controller API의 `CommitAndPinResult` 반환 타입 불일치, 테스트의 잔여 order/ID tie-break 표현 3건이 남았다. DB observation CAS와 command-scoped workbook mutation authorization barrier를 분리하고 반환 타입/테스트 문구를 통일한 뒤 최종 재검증한다.
- 후속 편집 완료 `doc/user_item_modify.txt`: `OutputDependencyObservationV1`을 DB transaction에서 재관측하는 `dbRecheckableObservation`과 pinned immutable workbook의 `frozenWorkbookObservation`으로 분리했다. freeze가 controller pin exclusion을 command-scoped `WorkbookSaveAuthorizationBarrier`로 원자 인계하고, pre-commit terminal failure 또는 post-commit resource recovery terminal에서만 해제하도록 본문·모델·테스트·완료 조건·구현 순서를 통일했다.
- 후속 편집 완료 `doc/user_item_modify.txt`: FortuneSheet API 반환을 controller-owned `CommitAndPinResult`로 통일하고 detach를 port-owned epoch failure로 분리했다. legacy replacer 테스트의 잔여 order/ID tie-break를 제거하고 app-owned guarded pop, strict session option 복원, sealed payload wire, cold durable apply/current-process live render ACK 테스트와 구현 순서를 모두 연결했다. 편집 직후 두 문서 diagnostics 오류 0건이다.
- 최종 검증 실행 예정: `get_errors(doc/user_item_modify.txt, SESSION_HANDOFF.md)`, 옛 order/GS1/close/observation/ACK 충돌 표현 검색과 새 계약 존재 검색, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md`, 변경 통계·상태 확인, 보완 3건 독립 승인 검토를 수행한다. 문서 계약만 변경했으므로 Flutter 테스트와 배포 빌드는 실행하지 않는다.
- 최종 검증 결과: 두 문서 diagnostics 오류 0건, 잔여 `persisted order/ID` 및 일반 `Navigator pop` 표현 0건, `BARCODE_TEXT_LINK` unique numeric order와 중앙 `requestGuardedPop()`/direct-pop lint 계약 5곳 확인, `git diff --check -- doc/user_item_modify.txt SESSION_HANDOFF.md` 통과다. 최종 독립 검토도 치명적/높음/중간 finding 없이 전체 승인했다. 문서 계약만 변경해 Flutter 테스트와 배포 빌드는 실행하지 않았다.
- stage/commit 대상: 첫 커밋은 `doc/user_item_modify.txt`만 `라벨 항목 편집기 작업지시서 20차 권장안 병합`으로 커밋한다. `.vscode/settings.json`, `lib/core/app.dart`, `SESSION_HANDOFF.md`는 첫 커밋 stage에서 제외한다.
- 커밋 완료: `doc/user_item_modify.txt`를 `3f6e871` (`라벨 항목 편집기 작업지시서 20차 권장안 병합`)로 로컬 커밋했다. 원격 push와 배포 작업은 수행하지 않았다.
- 임시 산출물/캐시: 생성하지 않았다. 마지막 stage/commit 대상은 `SESSION_HANDOFF.md` 한 파일이며 기존 unrelated `.vscode/settings.json`, `lib/core/app.dart`는 제외한다.
...
- 편집 완료 `lib/home_page_manager.dart`: `showItemManagerLoadFailureDialog`가 진행 스낵바 큐를 제거한 뒤 제목 `품목 조회 오류`, 네트워크 연결 확인 메시지, `확인` 버튼을 가진 AlertDialog를 표시한다. HomePageManager 상태 플래그로 동시 실패 시 다이얼로그 중복 표시를 막는다.
- 테스트 수정 `test/label_sheet_toolbar_test.dart`: 장기 진행 스낵바가 제거되고 경고 다이얼로그가 표시되며 `확인` 후 닫히는 실제 widget test로 변경했다. focused 테스트 통과, 변경 파일 diagnostics 오류 0건.
- 문서 수정 `doc/item_manager_modify.txt`: DB 조회 실패 계약을 오류 스낵바에서 경고 다이얼로그로 변경했다.
...
- 정리 완료: 테스트 생성물 `third_party/fortune_sheet/build` 삭제, `git diff --check` 통과, diff/stage 범위 확인 완료. 품목 조회 실패 경로의 `SnackBarType.error` 참조가 제거되고 세 실패 진입점이 경고 다이얼로그로 연결된 것을 확인했다.
- 기능 커밋: `0061002` (`품목 조회 오류를 경고창으로 표시`).
- 기존 unrelated dirty `lib/core/app.dart`는 수정·stage 대상에서 제외한다.

### 완료 (2026-07-12): DB 연결 실패 시 빈 품목 목록 무응답 보정
...
- 편집 완료 `lib/home_page_manager.dart`: 최초 브랜드 조회, 브랜드 변경 라벨 조회, 품목 세션 조회 실패가 공용 실패 안내로 수렴한다. 진행 스낵바와 큐를 제거한 뒤 `품목 데이터를 불러오지 못했습니다. 네트워크 연결을 확인한 뒤 다시 시도해 주세요.` 경고 다이얼로그를 표시한다. 최초 로드의 무조건 `hideCurrentSnackBar()`는 제거했으며 성공 종료는 기존 renderReady 경로가 담당한다.
