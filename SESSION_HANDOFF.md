...
## 현재 상태
- 진행 중: [doc/app_menu_porting.txt] 전체 구현. Phase 0·1 기반, Phase 2 전체, Phase 3 `협력업체 관리`까지 구현·검증·커밋했다. 다음은 Phase 3 `거래처 관리`다.
...
- Phase 3 다음 시작점: `거래처 관리`의 활성 레거시 `OnCustManage → CCustManagerDlg` resource→dialog/model→DAO와 실제 `BM_CUSTOMER` schema를 대조해 DB gate와 권한별 협력업체 selector 범위를 먼저 확정한다.
