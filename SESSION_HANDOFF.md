...
## 현재 상태
- 진행 중: 파일 popup의 `login/logout` metadata를 선두 section 0에서 `관리자 복사` 뒤·`종료` 바로 앞의 section 3으로 재배치했다. 관리 영역과 종료 그룹 사이 separator는 유지하고 현재 상태의 로그인/로그아웃과 종료 사이는 같은 section으로 묶는다. inventory는 `관리 4개 → 관리자 복사 → 로그인/로그아웃 → 종료` 순서를, widget test는 로그인 상태별 현재 command의 bottom과 종료 top이 맞닿고 중간 Divider가 없음을 검증한다. [doc/app_menu_porting.txt] 계약도 갱신했다. AppBar/inventory/controller/shortcut/lifecycle 회귀 `33 passed`, 수정 파일 diagnostics 0건, `C:\Flutter\bin\flutter.bat build windows --debug` 성공, `git diff --check` 통과. 관련 5개 파일만 stage/commit한다.
