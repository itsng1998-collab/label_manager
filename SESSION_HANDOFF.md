...
- 진행 중: [doc/app_menu_porting.txt] 전체 구현. Phase 0·1 기반, Phase 2 전체, Phase 3 관리 메뉴, 5.1.8 종료, 5.2.1 검색 및 치환과 5.3.1~5.3.3·5.3.5~5.3.6을 구현·검증·커밋했다. 5.2.2~5.2.6은 Phase 2에서 완료됐고 5.3.4는 `legacyUnreachable`, 5.3.7~5.3.8은 `legacyInactive` 제외이며 다음은 5.3.9 `업데이트 메시지`다.
...
- 5.3.1 품목별 정보 편집 진행 중: 기존 `ItemOfMarket`에 활성 출력 보정 field copyWith를 추가하고, 시장+품목 identity로 해당 field만 갱신하는 하나의 transaction API를 `ItemOfMarketDAO`에 추가했다. 숨김 판매·할인 field는 저장하지 않는다.
...
- 5.3.2 영양성분 형식 진행 중: `NutritionTypeDAO`에 manager 무정렬 목록, ID 순 template 목록, detail ID 순 조회와 신규·수정·삭제 transaction을 추가했다. 신규 parent는 `OUTPUT INSERTED` table variable로 받고 XML detail을 같은 statement에서 저장하며 `MAX`는 사용하지 않는다.
...
- 다음 작업: 5.3.11 검색출력 설정. 기존 column 전체 snapshot batch update 경로를 재사용하고 브랜드·label size selector, 검색 flag draft, dirty 폐기 확인, 적용 후 유지 동작을 연결한다.
- 5.3.11 검색출력 설정 진행 중: `SearchPrintSettingsDraft`와 전체 snapshot save command builder를 추가했다. 모든 조회 column을 updated list에 넣고 각 `persistedValues` 전체 key를 changed key로 전달해 기존 `LabelColumnSaveDao`의 main/min/GS1 transaction 전체 분기를 사용한다. flag-only UPDATE는 추가하지 않았다. focused 검증 전.
- 5.3.11 검색출력 설정 진행 중: command focused 3건 성공. ordered 브랜드/라벨 selector, column checkbox·전체 선택·전체 해제, dirty 전환/닫기 폐기 확인, Enter/적용, 성공 후 유지 dialog를 추가했다. menu owner는 기존 `LabelColumnSaveDao.saveDialogAndReload`로 전체 snapshot transaction 후 column을 reload해 committed baseline을 반환한다. touched diagnostics 검증 전.
...
- Phase 3 다음 시작점: `관리자 복사`의 활성 `CAdminCopyDlg`/observer/model/DAO를 대조해 일반 label size와 공용 label size 복사 transaction의 실제 삭제·복사 table 범위부터 확정한다.
