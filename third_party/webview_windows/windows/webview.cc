...
inline bool LooksLikeJsonValue(const std::string& s) {
  const std::string t = TrimAscii(s);
  if (t.empty()) return false;
  const unsigned char c = static_cast<unsigned char>(t[0]);
  if (c == '{' || c == '[' || c == '"' || c == '-' || std::isdigit(c)) return true;
  if (t.rfind("true", 0) == 0) return true;
  if (t.rfind("false", 0) == 0) return true;
  if (t.rfind("null", 0) == 0) return true;
  return false;
}

inline std::string JsonQuoteString(const std::string& s) {
  std::string out;
  ...
  webview_->add_WebMessageReceived(
      Callback<ICoreWebView2WebMessageReceivedEventHandler>(
          [this](ICoreWebView2* sender,
            ...
            // Prefer string path to avoid WebMessageAsJson parsing warnings
            // when the posted message is empty.
            wil::unique_cotaskmem_string wstr;
            if (args->TryGetWebMessageAsString(&wstr) == S_OK) {
              const std::string message = util::Utf8FromUtf16(wstr.get());
              const std::string trimmed = TrimAscii(message);
              if (!trimmed.empty()) {
                if (LooksLikeJsonValue(trimmed)) {
                  web_message_received_callback_(trimmed);
                } else {
                  web_message_received_callback_(JsonQuoteString(message));
                }
              }
              return S_OK;
            }

            wil::unique_cotaskmem_string wjson;
            if (args->get_WebMessageAsJson(&wjson) == S_OK) {
              const std::string message = util::Utf8FromUtf16(wjson.get());
...
bool Webview::PostWebMessage(const std::string& json) {
  ...
  if (TrimAscii(json).empty()) {
    // Avoid WebView2 'The document is empty' warnings for empty payloads.
    return false;
  }
