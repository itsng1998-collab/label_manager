...
void main() {
  ...
  testWidgets('image layer panel keyboard shift selects rows for delete', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(900, 700);
    tester.view.devicePixelRatio = 1;
    ...
    await tester.pump();
    expect(painter().activeImageId, 'image4');
    expect(painter().selectedImageIds, {'image4'});
    ...
    await tester.pump();
    expect(painter().activeImageId, 'image3');
    expect(painter().selectedImageIds, {'image4', 'image3'});
  ...
  testWidgets('image layer panel keyboard select all deletes rows', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(900, 700);
    tester.view.devicePixelRatio = 1;
  ...
  testWidgets('image layer panel keyboard select all duplicates rows', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(900, 700);
    tester.view.devicePixelRatio = 1;
    ...
    final duplicates = images.skip(3).toList();
    expect(painter().activeImageId, duplicates.last.id);
    expect(painter().selectedImageIds, {
      for (final duplicate in duplicates) duplicate.id,
    });
    expect(duplicates.map((image) => image.left), [12.0, 12.0, 12.0]);
    expect(duplicates.map((image) => image.top), [12.0, 12.0, 12.0]);
    expect(
      duplicates.map(
  ...
  testWidgets('image layer panel keyboard copies pastes and undoes rows', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(900, 700);
    tester.view.devicePixelRatio = 1;
    ...
    expect(images, hasLength(6));
    final pasted = images.skip(3).toList();
    expect(painter().selectedImageIds, {for (final image in pasted) image.id});
    expect(pasted.map((image) => image.left), [12.0, 12.0, 12.0]);
    expect(
      pasted.map((image) => image.extraFields[fortuneImageObjectIdExtraKey]),
      ['#IMAGE4', '#IMAGE5', '#IMAGE6'],
    );
    ...
    await tester.sendKeyUpEvent(LogicalKeyboardKey.controlLeft);
    await tester.pump();
    expect(painter().workbook.activeSheet.images, hasLength(3));
    ...
    expect(images, hasLength(6));
    expect(images.skip(3).map((image) => image.left), [12.0, 12.0, 12.0]);
  });

  testWidgets('image layer panel keyboard cuts and pastes rows', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(900, 700);
    tester.view.devicePixelRatio = 1;
    ...
    expect(images.map((image) => image.id), ['image1', 'image2', 'image3']);
    expect(images.map((image) => image.left), [12.0, 12.0, 12.0]);
    expect(painter().selectedImageIds, {'image1', 'image2', 'image3'});
  });
