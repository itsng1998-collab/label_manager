          ...
          children: [
            Expanded(
              child: Padding(
                padding: EdgeInsets.only(
                  left: 6,
                  right: state.swipeToggleVisible ? 2 : 6,
                ),
                child: Align(
                  alignment: Alignment.centerLeft,
                  child: Text(
...
class _ResizableTableState<T> extends State<ResizableTable<T>> {
  ...
  SwipeActionTableColumn<T> _toSwipeColumn(
    ResizableTableColumn<T> column,
    int columnIndex,
  ) {
    return SwipeActionTableColumn<T>(
      header: column.title,
      initialWidth: max(column.width, column.minWidth),
          ...
          return SizedBox(
            ...
            child: Center(
              child: Transform.scale(
                scale: 0.9,
                child: Checkbox(
                  value: safeIndex < _checked.length && _checked[safeIndex],
                  onChanged: (value) {
                    if (safeIndex >= _checked.length) return;
                    setState(() => _checked[safeIndex] = value ?? false);
                  },
